...
\includegraphics[width=0.55 \textwidth]{histos/Poisson_ctau2cm}
...
\includegraphics[width=0.45 \textwidth]{histos/d0_ctau2cm_pTcut20}


\caption{\label{fig:d0} d0 of electrons and muons. Left: no pT cuts, right: $pT=20 \text{ GeV}$ cut is applied}
\end{figure}
...
\begin{figure}[h!] \label{fig:d0-different-couplings}
\centering
\includegraphics[width=0.45 \textwidth]{histos/d0_ctau2cm}
\includegraphics[width=0.45 \textwidth]{histos/d0_ctau01}


\caption{ d0 of electrons and muons for different lifetimes.}
\end{figure}

Let's have a look on how does the d0 and l distribution changes with the change of the mass splittings: fig.({\ref{fig:d0-different-splittings}). As expected, both variables are larger for smaller mass splittings. Let's check the expected dependence for a couple of splittings (I don't include $\Delta m_{Cl}=10$ since there is not enough statiscs there):
\begin{align}
 &\frac{d0(\Delta m_{Cl}=30)}{d0(\Delta m_{Cl}=20)}\sim \frac{7}{15}\sim 0.47 \sim \left( \frac{2}{3} \right)^3=0.3\\
 &\frac{d0(\Delta m_{Cl}=40)}{d0(\Delta m_{Cl}=30)}\sim \frac{4}{7}\sim 0.57 \sim \left( \frac{4}{3} \right)^3=0.42\\
 &\frac{d0(\Delta m_{Cl}=50)}{d0(\Delta m_{Cl}=40)}\sim \frac{2}{4}\sim 0.5 \sim \left( \frac{4}{5} \right)^3=0.512\\
\end{align}


\begin{figure}[h!] {\label{fig:d0-different-splittings}
\centering
\includegrapLHhics[width=0.45 \textwidth]{histos/d0(e)_ctau2cm_different_splittings_pTcut}
\includegraphics[width=0.45 \textwidth]{histos/d0(mu)_ctau2cm_different_splittings_pTcut}
\includegraphics[width=0.55 \textwidth]{histos/l_ctau2cm_different_splittings}


\caption{d0 of electrons and muons and travel distance of the charged mediator for different mass splittings.}.
\end{figure}

Similarly, dependence on $m_C$ for fixed mass splitting is shown on fig.(\ref{fig:d0-different-mC}). Here I show d0 for electrons and muons only to show that they look the same. As expected, the boost factor $\beta \gamma$ is larger for smaller mediator masses which also leads to larger travel distance $l$. However, the impact on d0 is much smaller than for $\Delta m_{Cl}$.

\begin{figure}[h!] {\label{fig:d0-different-mC}
\centering
\includegraphics[width=0.45 \textwidth]{histos/d0(e)_ctau2cm_different_mC_pTcut}
\includegraphics[width=0.45 \textwidth]{histos/d0(mu)_ctau2cm_different_mC_pTcut}
\includegraphics[width=0.45 \textwidth]{histos/l_ctau2cm_different_mC}
\includegraphics[width=0.45 \textwidth]{histos/betagamma_ctau2cm_different_mC}
